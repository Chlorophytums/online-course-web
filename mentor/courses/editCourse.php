...
function updateCourse($id, $title, $price, $description, $label, $image, $videos, $conn)
{
    ...
    try {
        // Update data kursus di tabel tbl_courses
        $stmt = $conn->prepare("UPDATE tbl_courses SET title = ?, description = ?, price = ?, label = ? WHERE id = ?");
        $stmt->bind_param("ssdsi", $title, $description, $price, $label, $id);
        $stmt->execute();

        // Hapus semua media yang terkait dengan kursus dari tabel tbl_course_files
        $stmt_delete = $conn->prepare("DELETE FROM tbl_course_files WHERE course_id = ?");
        $stmt_delete->bind_param("i", $id);
        $stmt_delete->execute();

        // Siapkan statement untuk menyimpan media (video) ke dalam tabel tbl_course_files
        $stmt_media = $conn->prepare("INSERT INTO tbl_course_files (course_id, file_path, file_type) VALUES (?, ?, ?)");
...
if (isset($_GET['id'])) {
    ...
    // Ambil data kursus dari tabel tbl_courses
    ...
    {
        $stmt = $conn->prepare("SELECT file_path FROM tbl_course_files WHERE course_id = ? AND file_type = 'image'");
        $stmt->bind_param("i", $course_id);
        $stmt->execute();
        $result = $stmt->get_result();
    ...
    {
        $videos = array();
        $stmt = $conn->prepare("SELECT file_path FROM tbl_course_files WHERE course_id = ? AND file_type = 'video'");
        $stmt->bind_param("i", $course_id);
        $stmt->execute();
        $result = $stmt->get_result();
