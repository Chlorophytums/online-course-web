...
function addCourse($title, $price, $description, $label, $image, $videos, $conn)
{
    ...
    try {
        // Insert data kursus ke tabel tbl_courses
        $stmt = $conn->prepare("INSERT INTO tbl_courses (title, description, price, label, created_at) VALUES (?, ?, ?, ?, NOW())");
        $stmt->bind_param("ssds", $title, $description, $price, $label);
        $stmt->execute();
        $course_id = $stmt->insert_id;

        // Prepare statement untuk menyimpan media (gambar dan video) ke tabel tbl_course_files
